...
public class NetCDFParser extends AbstractParser {
    ...
    public void parse(InputStream stream, ContentHandler handler,
        ...
        NetCDFContentExtractor netCDFContentExtractor =
                context.get(NetCDFContentExtractor.class, new NetCDFDefaultContentExtractor());
        ...
        try {
            ...
            for (Variable var : ncFile.getVariables()) {
                ...
                if (netCDFContentExtractor.isContentExtractionEnabled()) {
                    xhtml.startElement("li");
                    xhtml.characters("data");
                    xhtml.newline();
                    try {
                        genDataXHTML(xhtml, var, netCDFContentExtractor.getDefaultVariableRange());
    ...
    private void genDataXHTML(XHTMLContentHandler xhtml, Variable variable, Range range)
            throws IOException, InvalidRangeException, SAXException {
        if (variable.getRank() == 1) {
            int numElements = variable.getDimension(0).getLength();
            List<Range> ranges = new ArrayList<Range>();
            if (numElements < range.length())
                ranges.add(0, new Range(numElements));
            else
                ranges.add(0, range);

            Array data = variable.read(ranges);
            xhtml.startElement("ul");
            while (data.hasNext()) {
                xhtml.element("li", data.next().toString());
            }
            xhtml.endElement("ul");
            ...
            for (int i = 0; i < numElements; i++) {
                xhtml.startElement("li");
                genDataXHTML(xhtml, variable.slice(0, i), range);
                xhtml.endElement("li");
            }
            xhtml.endElement("ul");
        }
